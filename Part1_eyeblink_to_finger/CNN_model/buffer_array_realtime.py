...
# buffer array to store incoming data during processing
buffer_data = []
...
while not finish:
    ...
    if msgtype == 1:
        ...
        # If currently processing, store incoming data in buffer_data
        if len(data1s) > channelCount * 400000 / samplingInterval:
            ...
            if len(data1s) > channelCount * 400000 / samplingInterval:
                ...
                data1s = data1s[leng//4:]
                t=[i for i in range(len(data1s))]
                a=np.reshape(np.array(plottoarr(t,data1s)),(1,48,64,1))
                if(model.predict(a,verbose='off')):
                    print("Blink detected")

                # Transfer data from buffer_data to data1s after processing
                buffer_data=buffer_data[leng:]
                data1s.extend(buffer_data)
                buffer_data = []
